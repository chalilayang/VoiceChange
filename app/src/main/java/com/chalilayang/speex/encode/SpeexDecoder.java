...
import com.chalilayang.speex.writer.OggCrc;
...
public class SpeexDecoder {
    ...
    private void initializeAndroidAudio(int sampleRate, int streamMode) throws Exception {
        int minBufferSize = AudioTrack.getMinBufferSize(sampleRate, AudioFormat.CHANNEL_OUT_MONO,
                AudioFormat.ENCODING_PCM_16BIT);

        if (minBufferSize < 0) {
            throw new Exception("Failed to get minimum buffer size: " + Integer.toString
					(minBufferSize));
        }

        track = new AudioTrack(streamMode, sampleRate, AudioFormat.CHANNEL_OUT_MONO, AudioFormat
				.ENCODING_PCM_16BIT,
    ...
    public void decode(int streamMode) throws Exception {
        ...
        try {
            ...
            while (true) {
                ...
                for (curseg = 0; curseg < segments; curseg++) {
                    ...
                    if (packetNo == 0) {
                        if (readSpeexHeader(payload, 0, bodybytes, true, streamMode)) {
     ...
     * @throws Exception
     */
    private boolean readSpeexHeader(final byte[] packet, final int offset, final int bytes, boolean init, int streamMode) throws Exception {
        if (bytes != 80) {
            return false;
        }
        ...
        initializeAndroidAudio(sampleRate, streamMode);
